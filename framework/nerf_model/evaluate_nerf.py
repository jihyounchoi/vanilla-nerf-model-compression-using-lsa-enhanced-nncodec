...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    if raw_noise_std > 0.:
        noise = torch.randn(raw[...,3].shape).device() * raw_noise_std
...
def train(nerf_wrapper, device, basedir_save, basedir, datadir, 
          ...
          i_print=100, i_img=500, i_weights=10000, i_testset=50000, i_video=50000):
    ...
    if render_only:
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir_save, 'renderonly_{}_{:06d}'.format('test' if render_test else 'path', start))
    ...
    rgb, disp, acc, extras = render(H, W, K, chunk=chunk, rays=batch_rays, 
                                    verbose=False, retraw=True, **render_kwargs_train)

    img_loss = img2mse(rgb, target_s)
    loss = img_loss
    psnr = mse2psnr(img_loss)
    
    return psnr.item(), loss.item()
