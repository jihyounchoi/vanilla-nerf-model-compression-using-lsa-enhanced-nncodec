...
def batchify(fn, chunk):
    ...
    def ret(inputs):
        ...
        for i in range(0, inputs.shape[0], chunk):
            result_list.append(fn(inputs[i:i+chunk]))
...
def create_nerf(nerf_wrapper, multires, i_embed, use_viewdirs, multires_views, 
    ...
    render_kwargs_train = {
        ...
        'perturb' : perturb,
        'N_importance' : N_importance,
        'network_fine' : nerf_wrapper.model_fine,
        'N_samples' : N_samples,
        'network_fn' : model,
        'use_viewdirs' : use_viewdirs,
        'white_bkgd' : white_bkgd,
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)), (1.-alpha + 1e-10)], -1), -1)[:, :-1] # alpha * T_i
...
def train(nerf_wrapper, basedir_save, basedir, datadir, i_save, N_iters,
    ...
    learning_rate = nerf_wrapper.tuning_optimizer.param_groups[0]['lr']
